    ...
    async def _make_request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                raise McpError(ErrorData(code=e.response.status_code, message=f"API request failed: {e.response.status_code} - {e.response.text}"))
            except Exception as e:
                raise McpError(ErrorData(code=-1, message=f"Request failed: {str(e)}"))
...
async def list_incidents(
    ...
    try:
        ...
        raise McpError(ErrorData(code=-1, message=f"Failed to list incidents: {str(e)}"))
...
async def get_incident(incident_id: str) -> str:
    ...
    try:
        ...
        raise McpError(ErrorData(code=-1, message=f"Failed to get incident {incident_id}: {str(e)}"))
...
async def create_incident(
    ...
    try:
        ...
        raise McpError(ErrorData(code=-1, message=f"Failed to create incident: {str(e)}"))
...
async def list_users(page_size: int = 25, after: Optional[str] = None) -> str:
    ...
    try:
        ...
        raise McpError(ErrorData(code=-1, message=f"Failed to list users: {str(e)}"))
...
async def list_severities() -> str:
    ...
    try:
        ...
        raise McpError(ErrorData(code=-1, message=f"Failed to list severities: {str(e)}"))
...
async def list_incident_statuses() -> str:
    ...
    try:
        ...
        raise McpError(ErrorData(code=-1, message=f"Failed to list incident statuses: {str(e)}"))
